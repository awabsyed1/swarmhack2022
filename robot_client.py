...
def naive(robot):
    print(f"{robot.mode}: {robot.state}")

    if master_mind_mode:
        if(robot.goal):
            return move_to(robot, robot.goal)
        else:
            return factory_movement(robot)

    decide_state(robot)
    if(robot.mode == RobotMode.FACTORY):
        left, right = factory_movement(robot)
    elif(robot.mode== RobotMode.APPROACH):
        left, right = approach_movement(robot)
    elif(robot.mode== RobotMode.FOLLOWING):
        left, right = move_to(robot, robot.goal)
    else:
        left = 0
        right = 0
    return left, right

def slave_driver(robot :Robot):
    if is_master(robot):
        return 0,0
    
    else:
        # slaves follow master
        if str(master_id) in list(robot.neighbours.keys()):
            print(f"{robot.id} following master")
            return move_to(robot, robot.neighbours[str(master_id)])
        else:
            print(f"{list(robot.neighbours.keys())}")
            return naive(robot)

def factory_movement(robot):
    if robot.state == RobotState.FORWARDS:
        left = right = robot.MAX_SPEED
        if (time.time() - robot.turn_time > 3) and any(ir > robot.ir_threshold for ir in robot.ir_readings):
...
def approach_movement(robot :Robot):
    # logic for custom task approach behaviour
    # find best task to approach
    nearest_task = list(robot.tasks.values())[0]
    for key, task in robot.tasks.items():
        if task["range"] < nearest_task["range"]:
            print(f"Nearest task workers: {nearest_task['workers']}")
            nearest_task = task

    return move_to(robot, nearest_task)

    # in other cases, just wait
    return 0, 0

def move_to(robot, goal):
    # option 2
    return move_to2(robot, goal)
    # face the task
    if(abs(goal["bearing"]) > bearing_threshold):
        if(goal["bearing"] < 0):
            return -robot.MAX_SPEED, robot.MAX_SPEED # turn left
        else:
...
def move_to2(robot, goal):
    if robot.state == RobotState.FORWARDS:
        # face the task
        if(abs(goal["bearing"]) > bearing_threshold):
            if(goal["bearing"] < 0):
                robot.state = RobotState.LEFT
                robot.time_to_turn = .1
                robot.turn_time = time.time()
                return -robot.MAX_SPEED, robot.MAX_SPEED # turn left
            else:
...
def decide_state(robot :Robot):
    # primarily help neighbours
    for neighbour_id, neighbour in robot.neighbours.items():
        if not robot.needs_help and int(neighbour_id) in list(active_robots.keys()) and (active_robots[int(neighbour_id)].needs_help):
            print(f"{robot.id} Wants to follow {neighbour_id}")
            robot.mode = RobotMode.FOLLOWING
            robot.needs_help = True
            robot.goal = neighbour
            return

    robot.goal = {}
    if len(robot.tasks) > 0:
        robot.needs_help = True
        robot.mode = RobotMode.APPROACH
    else:
        robot.needs_help = False
        robot.mode = RobotMode.FACTORY

def delegate_goals():
    solvable_tasks = find_solvable_tasks()
    active_bots = list(active_robots.keys())
    for id, robot in active_robots.items():
        # delete any previous goals
        robot.goal = {}
    
    for task_id, task in solvable_tasks.items:
        for id in active_bots:
            if task_id in list(active_robots[id].tasks.items()):
                active_robots[id].goal = task
                active_bots.remove(id)
            elif does_robot_chain_know_task(active_robots[id], task_id):
                g = {}
                for nid, n in active_robots[id]:
                    if task_id in list(active_robots[nid].tasks.keys()):
                        g = n
                        break
                active_robots[id].goal = g
                active_bots.remove(id)

def find_solvable_tasks():
    # initialise with the set of all known solvable tasks
    all_tasks = get_known_tasks()
    solvable_tasks = {}
    for task_id, task in all_tasks.items():
        # trace outward from goal to robots that know it
        robots_that_know_task = set()
        for id, robot in active_robots.items():
            if does_robot_chain_know_task(robot, task_id):
                robots_that_know_task.add(robot.id)
        
        if len(robots_that_know_task) >= task["workers"]:
            solvable_tasks[task_id] = all_tasks[task_id]

    return solvable_tasks

def does_robot_chain_know_task(robot, task_id, explored_robots = set()):
    if task_id in list(robot.tasks.keys()):
        return True
    else:
        explored_paths = 0
        explored_robots.add(robot)
        for key, neighbour in robot.neighbours.items():
            if key in list(active_robots.keys()) and not active_robots(key) in explored_robots:
                return does_robot_chain_know_task(active_robots(key), task_id, explored_robots)
        if(explored_paths == 0):
            return False
...
def get_known_tasks():
    ...
    for id, robot in active_robots.items():
        for task_id, task in robot.tasks.items():
            task_set[task_id] = task
    return task_set

def is_master(robot):
    return robot.id == master_id
...
async def send_commands(robot):
    try:
        ...
        if robot.teleop:
            ...
            # Autonomous mode
            global master_exists
            if active_robots[master_id].teleop:
                master_exists = True
            else:
                master_exists = False
            if not master_exists:
                left, right = naive(robot)
            else:
                left, right = slave_driver(robot)
                ...
                if key == "\r":
                    ...
                    try:
                        if int(robot_id) in valid_robots:
                            ...
                            global master_id
                            master_id = int(robot_id)
                            print(f"Set master to: {robot_id}")
